...
//creating a tree that specifies the simulation conditions
    ...
    struct {
       Int_t nnu; //this struct modified to fit all input.txt parameters into sim_parameters tree
      Int_t seed;
      Double_t exponent;
       Double_t atgap;
      Int_t st_type;
      ...
      Double_t z;
      Double_t icethick;
      //firn below (firn_presence)
      Double_t nfirn;
      Double_t firndepth;
       Int_t nrows;
       Int_t ncols;
      Bool_t scatter;
      Double_t scatter_width;
      //spectrum below
      Bool_t dipole;//Dipole:???
      //const_attenlength below
      Double_t atten_up;
      Double_t atten_down;
      Double_t nsigma;
      Double_t atten_factor;
      Double_t reflect_rate;
      //GZK flux below
      Int_t fanflux;
      //widespectrum below
      //shadowing below: shadowing_dir_ref
      //DEPTH_DEPENDENT_N below (depthdependentfirn)
      Bool_t hexagonal;
      //signal fluct below
      Float_t gainv;
      //tauregen below
      Double_t st4_r; //radius in meters between center of station and antenna
      Double_t tnoise;
      Double_t freq_low;
      Double_t freq_high;
      ...
      Double_t stationdepth;
   } b5;

//------------------------------------------------------------------+
   TTree* tree5 = new TTree("sim_parameters", "a tree of all simulation parameters"); 
//------------------------------------------------------------------+
    tree5->Branch("nnu", &b5.nnu, "nnu/I");
    tree5->Branch("seed", &b5.seed, "seed/I");
    tree5->Branch("exponent", &b5.exponent, "exponent/D");
    tree5->Branch("atgap", &b5.atgap, "atgap/D");
    tree5->Branch("st_type", &b5.st_type, "st_type/I");
    tree5->Branch("N_Ant_perST", &b5.N_Ant_perST, "N_Ant_perST/I");
    tree5->Branch("N_Ant_Trigger", &b5.N_Ant_Trigger, "N_Ant_Trigger/I");
    tree5->Branch("z", &b5.z, "z/D");
    tree5->Branch("icethick", &b5.icethick, "icethick/D");
    tree5->Branch("firn_presence", &b5.firn_presence, "firn_presence/O");
    tree5->Branch("nfirn", &b5.nfirn, "nfirn/D");
    tree5->Branch("firndepth", &b5.firndepth, "firndepth/D");
    tree5->Branch("nrows", &b5.nrows, "nrows/I");
    tree5->Branch("ncols", &b5.ncols, "ncols/I");
    tree5->Branch("scatter", &b5.scatter, "scatter/O");
    tree5->Branch("scatter_width", &b5.scatter_width, "scatter_width/D");
    tree5->Branch("spectrum", &b5.spectrum, "spectrum/O");
    tree5->Branch("dipole", &b5.dipole, "dipole/O");
    tree5->Branch("const_attenlength", &b5.const_attenlength, "const_attenlength/I");
    tree5->Branch("atten_up", &b5.atten_up, "atten_up/D");
    tree5->Branch("atten_down", &b5.atten_down, "atten_down/D");
    tree5->Branch("nsigma", &b5.nsigma, "nsigma/D");
    tree5->Branch("atten_factor", &b5.atten_factor, "atten_factor/D");
    tree5->Branch("reflect_rate", &b5.reflect_rate, "reflect_rate/D");
    tree5->Branch("gzk_flux", &b5.gzk_flux, "gzk_flux/O");
    tree5->Branch("fanflux", &b5.fanflux, "fanflux/O");
    ...
    tree5->Branch("depthdependentfirn", &b5.depthdependentfirn, "depthdependentfirn/O");
    tree5->Branch("hexagonal", &b5.hexagonal, "hexagonal/O");
    tree5->Branch("signal_fluctuation", &b5.signal_fluctuation, "signal_fluctuation/O");
    tree5->Branch("gainv", &b5.gainv, "gainv/F");
    tree5->Branch("tau_regeneration", &b5.tau_regeneration, "tau_regeneration/O");
    tree5->Branch("st4_r", &b5.st4_r, "st4_r/D");
    tree5->Branch("tnoise", &b5.tnoise, "tnoise/D");
    tree5->Branch("freq_low", &b5.freq_low, "freq_low/D");
    tree5->Branch("freq_high", &b5.freq_high, "freq_high/D");
    tree5->Branch("station_depth", &b5.stationdepth, "stationdepth/D");


    //tree5->Branch("atgap", &b5.atgap, "atgap/D"); moved above to preserve order of input parameters
    //   tree5->Branch("nrows", &b5.nrows, "nrows/I"); moved above
    //   tree5->Branch("ncols", &b5.ncols, "ncols/I"); moved above
 ...
 //   tree5->Branch("signal_fluctuation", &b5.signal_fluctuation, "signal_fluctuation/O");
    ...
    //   tree5->Branch("firn_presence", &b5.firn_presence, "firn_presence/O"); moved above
    //   tree5->Branch("spectrum", &b5.spectrum, "spectrum/O"); moved above
    //   tree5->Branch("widespectrum", &b5.widespectrum, "widespectrum/O");
   //   tree5->Branch("gzk_flux", &b5.gzk_flux, "gzk_flux/O"); moved above
    ...
    //   tree5->Branch("const_attenlength", &b5.const_attenlength, "const_attenlength/I"); moved above
    //    tree5->Branch("shadowing_dir_ref", &b5.shadowing_dir_ref, "shadowing_dir_ref/I"); moved above
...
// 05/09/2011 KD: leaving NNU as absolute given number for the moment for multi-station
   ...
   b5.nnu = NNU;
   b5.seed = seed;
   b5.exponent = EXPONENT;
   b5.atgap = ATGap; //make sure to change to float if required
   b5.st_type = ST_TYPE;
   b5.N_Ant_perST = N_Ant_perST;
   b5.N_Ant_Trigger = N_Ant_Trigger;
   b5.z = Z;
   b5.icethick = ICETHICK;
   b5.firn_presence = FIRN;
   b5.nfirn = NFIRN;
   b5.firndepth = FIRNDEPTH;
   b5.nrows = NROWS;
   b5.ncols = NCOLS;
   b5.scatter = SCATTER;
   b5.scatter_width = SCATTER_WIDTH;
   b5.spectrum = SPECTRUM;
   b5.dipole = DIPOLE;
   b5.const_attenlength = CONST_ATTENLENGTH;
   b5.atten_up = ATTEN_UP;
   b5.atten_down = ATTEN_DOWN;
   b5.nsigma = NSIGMA;
   b5.atten_factor = ATTEN_FACTOR;
   b5.reflect_rate = REFLECT_RATE;
   b5.gzk_flux = GZK;
   b5.fanflux = FANFLUX;
   b5.widespectrum = WIDESPECTRUM;
   b5.shadowing_dir_ref = SHADOWING;
   b5.depthdependentfirn = DEPTH_DEPENDENT_N;
   b5.hexagonal = HEXAGONAL;
   b5.signal_fluctuation = SIGNAL_FLUCT;
   b5.gainv = gainv;
   b5.tau_regeneration = TAUREGENERATION;
   b5.st4_r = ST4_R;
   b5.tnoise = TNOISE;
   b5.freq_low = FREQ_LOW;
   b5.freq_high = FREQ_HIGH;
    ...
    b5.stationdepth = STATION_DEPTH;

    //tree5->Fill(); //moved downstairs to include Veff
...
//KD I'm not sure equation is used in the right way here
   ...
   TTree* tree9 = new TTree("Veff", "A tree for the effective volume");

   //Below is for separate branches
   Double_t Veff_log = log10(Veff);
   Double_t veffe_log = log10(Veff_nue);
   Double_t veffmu_log = log10(Veff_numu);
   Double_t vefftau_log = log10(Veff_nutau);

   tree9->Branch("Energy_exponent", &EXPONENT, "EXPONENT/D");
   tree9->Branch("Veff", &Veff, "Veff/D");
   tree9->Branch("Veff_log", &Veff_log, "Veff_log/D");
   tree9->Branch("Veff_nue", &Veff_nue, "Veff_nue/D");
   tree9->Branch("veffe_log",&veffe_log, "veffe_log/D");
   tree9->Branch("Veff_numu", &Veff_numu, "Veff_numu/D");
   tree9->Branch("veffmu_log", &veffmu_log, "veffmu_log/D");
   tree9->Branch("Veff_nutau", &Veff_nutau, "Veff_nutau/D");
   tree9->Branch("vefftau_log", &vefftau_log, "vefftau_log/D");

   tree5->Branch("Energy_exponent", &EXPONENT, "EXPONENT/D");
   tree5->Branch("Veff", &Veff, "Veff/D");
   tree5->Branch("Veff_log", &Veff_log, "Veff_log/D");
   tree5->Branch("Veff_nue", &Veff_nue, "Veff_nue/D");
   tree5->Branch("veffe_log",&veffe_log, "veffe_log/D");
   tree5->Branch("Veff_numu", &Veff_numu, "Veff_numu/D");
   tree5->Branch("veffmu_log", &veffmu_log, "veffmu_log/D");
   tree5->Branch("Veff_nutau", &Veff_nutau, "Veff_nutau/D");
   tree5->Branch("vefftau_log", &vefftau_log, "vefftau_log/D");

   tree5->Fill();

   //Below is for single branch
   /*
   struct{
     Double_t exponent;
     Double_t nuenergy;
   ...
   tree9->Branch("Energy_exponent", &b9.exponent, "exponent/D:nuenergy:Veff:Veff_nue:Veff_numu:Veff_nutau");

   b9.exponent = EXPONENT;
   b9.nuenergy = pow(10,EXPONENT);
   b9.Veff = Veff;
   b9.Veff_nue = Veff_nue;
   b9.Veff_numu = Veff_numu;
   b9.Veff_nutau = Veff_nutau;
   */
